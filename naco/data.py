...
def return_quad(datadir,hexpt,bpm,sort_in_quads=True):
...
def ident_fluxframes(filetable,sc_eq_fl=False):
...
#        allsmed.append(np.median(sdata,axis=0)) images already med
    ...
    ascii.write(filetable,output=os.path.join(intermdir,'filetable_bkgrnd.csv'),delimiter=',',
                overwrite=True)
    print('Done Removing background. Stored results in {}'.format(intermdir))
#    return filetable
