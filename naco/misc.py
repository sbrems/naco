...
def read_fits(directory,verbose=True,only_first=False,cam=None,
              skip_last=False,only_last=False):
    ...
    for fl in files:
        if fl.endswith('.fits'):
            ...
            if cam != None:
                ...
                if not ((len(hdulist) ==1) or \
                        (len(hdulist) ==4)): raise ValueError('Caution! Unknown file format at 2file %s'%fl)
                header = hdulist[0].header
                data = hdulist[0].data
            #check data shape
            if (len(data.shape) == 3) & (only_first):
                data = data[0,:,:]
            elif (len(data.shape) == 3) & (skip_last):
                data = data[0:-1,:,:]
